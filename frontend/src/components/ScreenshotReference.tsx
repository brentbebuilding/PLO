      ...
      {showRankPicker && detectedSuit && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-xl p-4 max-w-sm w-full">
            ...
            {/* Colour sampling lands on the wrong suit often enough that it
                needs an override here, not a "try clicking again" hint. */}
            <div className="mt-4 border-t border-gray-700 pt-3">
              <div className="text-gray-400 text-xs mb-2">Wrong suit? Set it directly:</div>
              <div className="flex gap-2">
                {SUITS.map((suit) => (
                  <button
                    key={suit}
                    onClick={() => setDetectedSuit(suit)}
                    className={`flex-1 py-2 rounded text-xl bg-white hover:bg-gray-100 ${getSuitColor(suit)} ${
                      detectedSuit === suit ? 'ring-2 ring-green-400' : ''
                    }`}
                  >
                    {SUIT_SYMBOLS[suit]}
                  </button>
